...
import org.adempiere.webstore.util.WebSessionCtx;
import org.adempiere.webstore.util.WebstoreUtil;
    ...
    extends HttpServlet
{
    ...
    //	WEnv.dump(request);
		...
		org.adempiere.webstore.util.WebLogin thisLogin = new org.adempiere.webstore.util.WebLogin(request, response, ctx);
		thisLogin.init ();
        ...
        {
            WebstoreUtil.reload("User not logged in", LOGIN_JSP, session, request, response, getServletContext());
            return;
        }
		if (!thisLogin.action ())
		{
			WebstoreUtil.reload(thisLogin.getMessage(), thisLogin.getUpdate_page (), session, request, response, getServletContext());
			return;
		}
